...
DATE=$(find . -name 'changesets-*.osm.bz2' | sort | tail -n1 | ggrep -Po "(?<=^./changesets-)2\d\d\d\d\d(?=.osm.bz2$)")

echo "Extracting StreetComplete changeset ids"
if [ "changesets-${DATE}.osm.bz2" -nt "${DATE}-changesets.txt.gz" ] ; then
	osmium cat -f opl "changesets-${DATE}.osm.bz2" | pv -s 100M  -l -c -N "Input" | ggrep -P " T(\S+,)?created_by=Street(%20%)?Complete%20%" | cut -d" " -f1 | cut -c2- | pv -l -c -N "Num. output changesets" | gzip > "${DATE}-changesets.txt.gz"
fi
...
if [ "${DATE}-changesets.txt.gz" -nt osm-edits.db ] ; then
	echo -e "create table sc_changesets ( id INTEGER );\n.mode csv\n.import \"|zcat < ${DATE}-changesets.txt.gz\" sc_changesets\n" | sqlite3 osm-edits.db
fi
